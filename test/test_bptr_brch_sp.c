...
void _bptr_part_full_brch_casc_create(struct bptr_temp *temp)
{
   ...
   snprintf(path, sizeof(path), "bptr_files/temp_part_casc/%s", temp->fnm);
   mkdir("bptr_files", 0755);
   mkdir("bptr_files/temp_part_casc", 0755);
   if (access(path, F_OK) == 0) return;
   bptr = bptr_init(path, temp->is_lite, temp->node_sz, temp->key_sz,
                    temp->val_sz, temp->cache_cap, temp->cmp);
   TEST_ASSERT_MESSAGE(bptr, "failed at bptr_init");
   ...
   // Build a height-3 tree where leaves and L1 are full but the root (L2)
   // is NOT full — it has only 1 key (2 L1 children) instead of brch_full
   // keys. This ensures a cascading split from leaf→L1 will stop at the
   // root without creating a new root (height stays at 3).
   //
   // IMPORTANT: bptr_node_new(bptr, 0) assigns level = self->height++,
   // so nodes MUST be created bottom-up: leaf first, then L1, then root.
   ...
   // Track last leaf of first L1 for cross-L1 leaf chain linkage
   bptr_node_t prev_l1_last_leaf = node->node_idx;
   bptr_node_unload(bptr, node);

   // ---- Second (and last) L1 subtree ----
   {
      struct bptr_node *prev_l1 = l1_n;
      l1_n = bptr_node_new(bptr, root_n->node_idx);
      TEST_ASSERT_NOT_NULL_MESSAGE(l1_n, "bptr_node_new failure");
      ...
      // First leaf of second L1 — link to previous L1's last leaf
      node = bptr_node_new(bptr, l1_n->node_idx);
      TEST_ASSERT_NOT_NULL_MESSAGE(node, "bptr_node_new failure");
      {
         struct bptr_node *prev_last =
      ...
      // Promote second L1 into root: key = first key of L1's first leaf.
      // Root gets exactly 1 key (2 children total).
      {
         struct bptr_node *fl = bptr_node_fetch(bptr, _node_brch_vals_get(bptr, l1_n, 0));
         TEST_ASSERT_NOT_NULL_MESSAGE(fl, "failed to fetch first leaf");
         _bptr_kv_ins_i64(root_n, temp->tools,
                          temp->tools->node.cast_i64(fl->keys),
                          l1_n->node_idx, 0, bptr->is_lite);
         bptr_node_unload(bptr, fl);
      }
    }
   l1_n->next = 0;
...
void _bptr_part_full_brch_casc_verify(struct bptr_temp *temp)
{
   ...
   // Verify root: NOT full, has exactly 1 key (2 children)
   root_n = bptr_node_fetch(bptr, bptr->root_idx);
   TEST_ASSERT_NOT_NULL_MESSAGE(root_n, "failed to fetch root");
   TEST_ASSERT_FALSE_MESSAGE(root_n->is_leaf, "root should not be leaf");
   ...
   TEST_ASSERT_EQUAL_MESSAGE(2, root_n->level, "root level != 2");
   TEST_ASSERT_EQUAL_UINT64_MESSAGE(0, root_n->parent, "root parent != 0");
   TEST_ASSERT_EQUAL_UINT64_MESSAGE(0, root_n->prev, "root prev != 0");
   TEST_ASSERT_EQUAL_UINT64_MESSAGE(0, root_n->next, "root next != 0");
   TEST_ASSERT_BITS_HIGH_MESSAGE(BPTR_NODE_FLAG_VALID, root_n->flags,
                                  "root flags missing VALID");
   TEST_ASSERT_BITS_LOW_MESSAGE(BPTR_NODE_FLAG_LEAF, root_n->flags,
                                 "root flags has LEAF set");
   ...
   // Verify node and record counts
   {
      uint64_t expected_node_cnt = 5 + 2 * (uint64_t)brch_full;
      uint64_t expected_record_cnt =
         2 * ((uint64_t)brch_full + 1) * leaf_full;
      TEST_ASSERT_EQUAL_UINT64_MESSAGE(expected_node_cnt, bptr->node_cnt,
         "node_cnt mismatch");
      TEST_ASSERT_EQUAL_UINT64_MESSAGE(expected_record_cnt, bptr->record_cnt,
         "record_cnt mismatch");
   ...
   // Verify each L1 and its leaves (2 L1 subtrees)
   for (uint32_t root_i = 0; root_i <= 1; root_i++)
    {
      l1_n = bptr_node_fetch(bptr, _node_brch_vals_get(bptr, root_n, root_i));
      TEST_ASSERT_NOT_NULL_MESSAGE(l1_n, "failed to fetch L1 node");
      TEST_ASSERT_FALSE_MESSAGE(l1_n->is_leaf, "L1 should not be leaf");
      ...
      if (root_i == 0)
       {
         TEST_ASSERT_EQUAL_UINT64_MESSAGE(0, l1_n->prev, "first L1 prev != 0");
         TEST_ASSERT_EQUAL_UINT64_MESSAGE(
            _node_brch_vals_get(bptr, root_n, 1), l1_n->next,
            "first L1 next linkage incorrect");
       }
       ...
       {
         TEST_ASSERT_EQUAL_UINT64_MESSAGE(
            _node_brch_vals_get(bptr, root_n, 0), l1_n->prev,
            "second L1 prev linkage incorrect");
         TEST_ASSERT_EQUAL_UINT64_MESSAGE(0, l1_n->next,
                                           "last L1 next != 0");
       }
   ...
   // Verify root separator key matches first key of second L1's first leaf
   {
      struct bptr_node *r_l1 = bptr_node_fetch(bptr,
         _node_brch_vals_get(bptr, root_n, 1));
      TEST_ASSERT_NOT_NULL_MESSAGE(r_l1,
         "failed to fetch second L1 for separator check");
      ...
      TEST_ASSERT_NOT_NULL_MESSAGE(r_first_leaf,
         "failed to fetch second L1's first leaf");
      TEST_ASSERT_EQUAL_INT64_MESSAGE(
         temp->tools->node.cast_i64(root_n->keys),
         temp->tools->node.cast_i64(r_first_leaf->keys),
         "root separator key does not match first key of right L1 subtree");
